...
void caricaVett(Cinema v[], int n, char nomeFilm[]) {
    FILE *fp; // puntatore al file che andrò ad utilizzare
    int c = 0;
    ...
    for(int i = 0; i < n; i++) {
        ...
        pezzo = strtok(line, ","); // l'indirizzo di pezzo sarà quello di line fino al carattere ',' che separa la linea in pezzi

        while(pezzo != NULL) { // quando pezzo varrà null il ciclo sifermerà perché arrivato alla fine del file

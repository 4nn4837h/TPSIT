...
NON si usi la funzione strcat della libreria stdio.h, si usiniìo i puntatiri e non la notazione vettoriale, attenzione al tappo '\0'
**/
...
void stampaStringa(char *s) {
    fflush(stdin);
    puts(s); // ogni prima lettera delle due stringhe stampa un carattere diverso, lo stesso: una 'l ad angolo' e la 'à'
}

void attacca(char *s1, char *s2, int n1, int n2) {
    //for(int k = 0; k < n1 - 1; k++)  // dimensione - 1 perché c'è il tappo
    int s = n1 + n2;
    *s1 = (char *)malloc(s * sizeof(char));
    int part = s - n2; // alla lunghezza totale della stringa tolgo la dimensione della seconda stringa così da trovare il suo punto di partenza per attaccarla
    strcpy((s1 + part), s2);
}
int main() {

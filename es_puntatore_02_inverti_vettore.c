...
void scambia(int *v, int n) {
    int t;
    for(int k = 0; k < n/2; k++) { // faccio un for fino a metà perché confronto a specchio
            t = *(v + k);
            *(v + k) = *(v + n - k - 1); // v + n-k-1: perché devo contare che parte da 0
            *(v + n - k - 1) = t; // scambio
    }
}
...
int main() {
    ...
    *v = (int*) malloc(dim*sizeof(int)); // inizializzazione del vettore con malloc: puntatore
    // il vettore (prima posizione, ovvero con inidice 0) è un int*, per cui faccio un casting, perché la maclloc restituisce un void*,
    // chiamo la funzione, moltiplico per quante celle voglio avere nel formato che voglio avere: è per avere il giusto numero di celle
